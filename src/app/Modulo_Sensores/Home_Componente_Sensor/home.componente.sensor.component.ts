...
export class HomeComponenteSensorComponent implements OnInit{
    ...
    ngOnInit(){
        this.gestionarComponenteSensorService.buscarComponenteSensorId(this.idComponenteSensor)
            ...
            //DEBERIA LLAMAR A UN REST QUE ME TRAIGA LOS SENSORES DEL COMPONENTE ACTUAL.
        /*this.homeComponenteSensorService.buscarSensoresNoAsignados(this.idFinca)
            .then(
                response=>{
                    if(response.detalle_operacion=="No hay datos"){
                        this.errorMessageSensor="No hay sensores disponibles para la finca seleccionada.";
                    }
                    else{
